...
export class JWTManager {
  ...
  async verifyToken(token, expectedType = 'access') {
    try {
      ...
      if (decoded.type !== expectedType) {
        throw new AuthenticationError(`无效的token类型，期望: ${expectedType}，实际: ${decoded.type}`);
      }
  ...
  cleanupExpiredTokens() {
    ...
    console.log(`[JWT] 清理完成，当前撤销token数量: ${this.revokedTokens.size}，活跃刷新token数量: ${this.refreshTokenStore.size}`);
  }
  ...
  getStats() {
    return {
      ...
      activeUsersCount: new Set([...this.refreshTokenStore.values()].map(info => info.userId)).size
    };
  }
}
...
export const authenticateToken = (requiredRoles = []) => {
  ...
  return async (req, res, next) => {
    try {
      ...
      if (!authHeader) {
        throw new AuthenticationError('缺少Authorization请求头');
      }

      const parts = authHeader.split(' ');
      if (parts.length !== 2 || parts[0] !== 'Bearer') {
        throw new AuthenticationError('无效的Authorization格式，应为: Bearer <token>');
      }
      ...
      if (requiredRoles.length > 0 && !requiredRoles.includes(decoded.role)) {
        throw new AuthorizationError(`需要以下角色之一: ${requiredRoles.join(', ')}`);
      }
