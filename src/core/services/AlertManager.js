...
class AlertManager {
  ...
  async loadAlertRules() {
    try {
      ...
      const rules = await db.raw('SELECT * FROM alert_rules WHERE is_active = 1');
      logger.info('Raw rules result:', rules);
      rules.forEach((rule) => {
        // 解析规则条件（JSON格式）
        try {
  ...
  async loadActiveAlerts() {
    try {
      ...
      logger.info('Raw alerts result:', alerts);
  ...
  async checkAlerts(data, dataType, deviceId) {
    ...
    for (const [, rule] of this.rules.entries()) {
      // 检查规则是否适用于此数据类型和设备
      if (rule.data_type !== dataType && rule.data_type !== 'all') {
        continue;
      }
  ...
  evaluateConditions(data, conditions) {
    try {
      ...
      if (conditions.type === 'compound') {
        const results = conditions.conditions.map((cond) => this.evaluateConditions(data, cond));
  ...
  async triggerAlert(rule, data, deviceId) {
    try {
      ...
      const existingAlertId = Array.from(this.activeAlerts.values()).find(
        (alert) =>
          alert.rule_id === rule.id && alert.device_id === deviceId && alert.status === 'active'
      )?.id;
    ...
    try {
      ...
      await db.raw(
        ...
        [
          ...
          updatedRule.data_type,
          updatedRule.device_id || null,
          JSON.stringify(updatedRule.conditions),
          JSON.stringify(updatedRule.actions || []),
          updatedRule.severity || 'medium',
          updatedRule.is_active !== false,
