...
export const deviceSchema = Joi.object({
  body: Joi.object({
    ...
    device_type: Joi.string().valid('energy_meter', 'carbon_sensor', 'battery', 'solar_panel', 'wind_turbine').required(),
...
export const alertRuleSchema = Joi.object({
  body: Joi.object({
    ...
    device_id: customValidators.deviceId.optional(),
    metric_type: Joi.string().valid('energy', 'carbon', 'battery_soc', 'battery_temperature').required(),
    condition: Joi.string().valid('>', '<', '>=', '<=', '==', '!=').required(),
    threshold_value: Joi.number().required(),
    severity: Joi.string().valid('low', 'medium', 'high', 'critical').required(),
    enabled: Joi.boolean().default(true),
    notification_channels: Joi.array().items(Joi.string().valid('email', 'sms', 'webhook')).min(1).required(),
...
export const reportGenerationSchema = Joi.object({
  body: Joi.object({
    report_type: Joi.string().valid('energy_consumption', 'carbon_emission', 'battery_performance', 'comprehensive').required(),
  ...
  return (req, res, next) => {
    ...
    for (const file of files) {
      ...
      if (allowedTypes.length > 0 && !allowedTypes.includes(file.mimetype)) {
        return next(new ValidationError(`不支持的文件类型: ${file.mimetype}，支持的类型: ${allowedTypes.join(', ')}`));
      }

      // 检查文件大小
      if (file.size > maxSize) {
        return next(new ValidationError(`文件大小超过限制: ${file.size} bytes，最大允许: ${maxSize} bytes`));
      }
...
export const validateContentType = (allowedTypes = ['application/json']) => {
  return (req, res, next) => {
    ...
    if (!isAllowed) {
      return next(new ValidationError(`不支持的Content-Type: ${contentType}，支持的类型: ${allowedTypes.join(', ')}`));
    }
