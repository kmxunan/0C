...
exports.up = async function(knex) {
  ...
  await knex.schema.createTable('recommendation_rules', (table) => {
    table.specificType('id', 'CHAR(36)').defaultTo(knex.raw('(UUID())')).primary().comment('规则ID');
    table.string('name').notNullable().comment('规则名称');
    table.text('description').comment('规则描述');
    table.string('type').notNullable().comment('规则类型: energy_saving, cost_reduction, comfort_improvement等');
  ...
  await knex.schema.createTable('recommendations', (table) => {
    table.specificType('id', 'CHAR(36)').defaultTo(knex.raw('(UUID())')).primary().comment('推荐ID');
    ...
    table.string('status').defaultTo('pending').comment('状态: pending, viewed, applied, dismissed');

    // 添加外键约束
    table.foreign('rule_id').references('id').inTable('recommendation_rules')
