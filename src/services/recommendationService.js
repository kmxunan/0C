...
class RecommendationService {
  ...
  static async _getEnergyConsumption(parkId, buildingId, startDate, endDate) {
    ...
    if (parkId) whereClause.park_id = parkId;
    ...
    const energyData = await EnergyData.findAll({
      where: whereClause,
      attributes: [
        [EnergyData.sequelize.fn('strftime', '%Y-%m-%d', EnergyData.sequelize.col('timestamp')), 'date'],
    ...
    Object.values(formattedData).forEach(dailyData => {
      Object.entries(dailyData).forEach(([type, value]) => {
        ...
        if (!dailyAverages[type]) dailyAverages[type] = [];
  ...
  static async _getCarbonEmissions(parkId, buildingId, startDate, endDate) {
    ...
    const carbonData = await CarbonData.findAll({
      where: whereClause,
      attributes: [
        [CarbonData.sequelize.fn('strftime', '%Y-%m-%d', CarbonData.sequelize.col('timestamp')), 'date'],
    ...
    const recentAvg = recentValues.length ? recentValues.reduce((sum, val) => sum + val, 0) / recentValues.length : 0;
      ...
      recommendations.map(async recommendation => {
        ...
        switch (recommendation.type) {
          ...
          default:
            ...
            implementationSteps = ['查看推荐详情', '评估实施难度', '制定实施计划', '执行推荐措施', '验证实施效果'];
        }
   ...
   */
  static async _estimateEnergySavings(recommendation) {
    // 这里应该有更复杂的估算逻辑
   ...
   */
  static async _calculateCostReductionDetails(recommendation) {
    // 这里应该有更复杂的计算逻辑
    return {
   ...
   */
  static async _estimateCostSavings(recommendation) {
    // 这里应该有更复杂的估算逻辑
  ...
  static async evaluateRecommendationEffectiveness(recommendationId, startDate, endDate) {
    // 获取推荐详情
    const recommendation = await db('recommendations').where({ id: recommendationId }).first();
    if (!recommendation) {
    ...
    switch (recommendation.type) {
      case 'energy_saving':
        evaluation = await this._evaluateEnergySavingEffectiveness(
          recommendation, preStartDate, preEndDate, startDate, endDate
        );
        break;
      case 'cost_reduction':
    ...
    // 保存评估结果
    await db('recommendation_evaluations').insert({
      id: uuidv4(),
