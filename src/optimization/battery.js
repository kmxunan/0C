...
// 加载训练数据
async function loadMockTrainingData() {
  const fs = require('fs').promises;
  const csv = require('csv-parser');
  const results = [];
  
  try {
    const data = await fs.readFile('/Users/xunan/Documents/WebStormProjects/0C/test-data/battery_data.csv', 'utf8');
...
export function setupBatteryRoutes(app, authenticateToken) {
  ...
  app.get('/battery/optimization', authenticateToken, async (req, res) => {
    try {
      ...
      // 调用优化模型
      const optimization = await optimizeBatteryStorage(buildingId, start_time, end_time, interval);
...
async function getCarbonIntensity(startTime, endTime) {
  try {
    ...
    for (let d = new Date(start); d <= end; d.setHours(d.getHours() + 1)) {
      const hour = d.getHours();
      // 模拟碳排放因子变化
      const intensity = 500 + Math.sin(hour/24*Math.PI*2) * 150 + (Math.random() * 100 - 50);
...
// 生成优化策略
async function generateOptimizationStrategies(predictions, interval, buildingId, startTime, endTime) {
...
// 准备预测输入数据
function preparePredictionInput(data, windowSize = 24) {
  if (!data || data.length === 0) {
...
// 获取历史储能数据（模拟）
function getHistoricalBatteryData(start_time, end_time, buildingId) {
  if (!start_time || !end_time) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const filteredData = batteryStatusData.filter(dataPoint => {
        ...
        return timestamp >= new Date(start_time).getTime() && 
...
// 预处理数据
function preprocessPredictionData(data) {
  const windowSize = 24;
  const xs = [];
  const ys = [];
  ...
  return {
    features: tf.tensor2d(xs, [xs.length, windowSize]),
...
async function trainBatteryOptimizationModel(model, features, labels) {
  ...
  console.log('模型训练完成，最终验证损失:', history.history.val_loss[history.history.val_loss.length - 1]);
  return model;
}
