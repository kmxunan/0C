...
const { HTTP_STATUS } = require('../../../src/shared/utils/apiResponse.js');
...
router.get('/predict/24hours', authenticate, async (req, res) => {
  try {
    ...
    if (temperature) {
      baseData.temperature = parseFloat(temperature);
    }
    if (humidity) {
      baseData.humidity = parseFloat(humidity);
    }
